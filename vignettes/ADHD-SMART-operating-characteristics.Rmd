...
$$
Y_{0,i}(a_1, a_2) = 2 - 0.2\tilde{odd}_i - 0.3 \tilde{severity}_i + \epsilon_i
$$

$$
Y_{1,i}(a_1, a_2) = 2.5 - 0.3a_1 + 0.4U_i + \epsilon_i
$$

$$
Y_{2,i}(a_1, a_2) = 3 - 0.3\bar{odd}_i - 0.3\bar{severity}_i + 0*\bar{priormed} + 0.5\bar{race}_i + \\
(0.3 - 1.4\tilde{priormed}_i)a_1 + \\
0.8\tilde{R} + 1 \tilde{Adh} \\
+(1-R)(-0.3 + 0.1a_1 + 1.2Adh_c)a_2 + 0.4U_i + \epsilon_i
$$
...
adhd.c <- adhd %>% mutate(across(c(2:5, adherence, event_time),  ~ .x - mean(.x, na.rm = TRUE), .names = "{.col}.c"), )
...
model1 <- adhd.c %>% geeglm(Y2 ~ A1 + odd.c + severity.c + priormed.c + race.c, 
                            id = ID, data = .)
summary(model1)
...
model2 <- adhd.nr.c %>% geeglm(Y2 ~ A2 + odd.c + severity.c + priormed.c + 
                                 race.c + event_time.c + adherence.c, id = ID, data = .)
summary(model2)
...
```{r}
modelMM <- geeglm(Y2 ~ A1*A2, weights = w, id = ID, data = adhd.rep)
summary(modelMM)
...
estimate(modelMM,
         rbind(
           ...
           "Diff: AI#3 - AI#4"              = c(0, -2,  0, -2))) %>% print()
...
```{r}
modelMM.adj <- adhd.rep %>% geeglm(Y2 ~ A1*A2 + odd.c + severity.c + priormed.c + race.c, 
...
model2s.c <- adhd.nr.c %>% geeglm(Y2 ~ odd.c + severity.c + priormed.c + race.c +
                                  event_time.c + adherence.c + 
...
em2 <- emmeans::emmeans(grid, ~ A1 + A2 | adherence) # This does not give a weighted grid!!!! use ref_grid or weights = "proportional"
summary(em2)
plot(em2)
...
model2s <-  adhd.c.nr %>%
  geeglm(Y2 ~ odd.c + severity.c + priormed.c + race.c + 
           event_time + adherence + A1*A2 + A2:adherence, 
         id = ID, data = .)
...
model1s <- adhd.c.opt %>% geeglm(Y2 ~ odd.c + severity.c + priormed.c + race.c +
...
em1 <- emmeans::emmeans(model1s, ~ A1 | priormed.c, weights = "proportional")
summary(em1)
plot(em1)
...
```{r}
# Can also calculate with lm and use sandwich package
model2s <- adhd.nr.c %>% lm(Y2 ~ odd.c + severity.c + priormed.c + race.c +
                                  adherence.c + A1*A2 + A2:adherence.c,
                                data = .)
summary(model2s)
sqrt(diag(sandwich::sandwich(model2s)))
```
